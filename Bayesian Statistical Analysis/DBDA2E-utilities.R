...
saveGraph = function( file="saveGraphOutput" , type="pdf" , ... ) {
  if ( .Platform$OS.type != "windows" ) { # Mac OS, Linux
    if ( any( type == c("png","jpeg","jpg","tiff","bmp")) ) {
      ...
      savePlot( file=paste0(file,".",type) , type=sptype , device = dev.cur())# Number of the device to be saved  
    }
    if ( type == "pdf" ) {
...
summarizePost = function( paramSampleVec , 
  ...
  ### TRIAL CODE FOR T-DISTRIBUTION
  tDistParams = fitdistr(paramSampleVec,"t",start = list(m=mean(paramSampleVec),s=sd(paramSampleVec),df=24))
  ...
  return( c( Mean=meanParam , Median=medianParam , Mode=modeParam , 
             TDistLoc = tDistParams$estimate[1], TDistShape = tDistParams$estimate[2],
             TDistDegFree = tDistParams$estimate[3], ESS=mcmcEffSz ,
...
plotPost = function( paramSampleVec , cenTend=c("mode","median","mean")[1] , 
  ...
  # My added code for not plotting the comp value or ROPE boundaries if they are
  # too far away from the HDI
  HDIwidth = HDI[2] - HDI[1]
  lowLimit = HDI[1] - 3*HDIwidth
  highLimit = HDI[2] + 3*HDIwidth
  xlim = range(c(lowLimit,highLimit))
  ...
  if ( !is.null( ROPE ) ) {
    ...
    # My correcting code
    if ((ROPE[1] > lowLimit) && (ROPE[1] < highLimit))
    {
      lines( c(ROPE[1],ROPE[1]) , c(0.96*ROPEtextHt,0) , lty="dotted" , lwd=2 ,
             col=ropeCol )
    }
    
    if ((ROPE[2] > lowLimit) && (ROPE[2] < highLimit))
    {
      lines( c(ROPE[2],ROPE[2]) , c(0.96*ROPEtextHt,0) , lty="dotted" , lwd=2 ,
             col=ropeCol)
    }
    
    if (mean(ROPE) < lowLimit)
    {
      xPos = lowLimit
    }
    else if (mean(ROPE) > highLimit)
    {
      xPos = highLimit
    }
    else
    ...
    text( xPos , ROPEtextHt ,
          bquote( .(round(100*pLtROPE,1)) * "% < " * .(ROPE[1]) * " < " * 
                   .(round(100*pInROPE,1)) * "% < " * .(ROPE[2]) * " < " * 
                   .(round(100*pGtROPE,1)) * "%" ) ,
          adj=c(pLtROPE+.5*pInROPE,0) , cex=1 , col=ropeCol )
    
    print(paste0("Printed ROPE red text at ",xPos,". Low limit was ",lowLimit," and high limit was ",highLimit))
