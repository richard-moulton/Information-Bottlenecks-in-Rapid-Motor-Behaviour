%% Additive Workload Serial System
%  A parallel-serial processing system where different tasks increase the
%  amount of work to be done by certain modules by an additive amount
%
%  3 December, 2021

function [finalRate,input_record,output_record] = AdditiveWorkloadSerialSystem(systemCapacities,taskModifiers,visualize)
...
moduleCapacities = systemCapacities + 0.30*randn([1 length(systemCapacities)]);
taskDifficulties = 1 - (taskModifiers + 0.01*randn([1 length(taskModifiers)]));
...
if visualize
    ...
    title(sprintf('Task (%s), Constant Reduction System (%s)',num2str(taskModifiers),num2str(systemCapacities)));
    xlabel('Time (s)');
